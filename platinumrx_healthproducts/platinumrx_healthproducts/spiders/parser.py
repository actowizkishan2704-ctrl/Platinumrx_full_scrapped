import scrapy
from platinumrx_healthproducts.pipelines import create_table
...
class ParserSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        to_cid = int(getattr(self, "to_id", 45901))
        cursor.execute("""SELECT id, products_urls FROM platinumrx_health_products_url WHERE status = 0 AND id BETWEEN %s AND %s ORDER BY id""",(from_cid, to_cid))
        ...
        cookies = {
            ...
            '_ga_BHBBVCGVE0': 'GS2.1.s1789361506$o17$g1$t1789362342$j60$l0$h1716316719$da4THJHUtYw8-09PKcD8CrqgcohCDFd1nIw',
            '_dd_s_v2': 'aid=47a24d2b-491d-423a-9130-315a19c20816&id=9080f951-f0ce-41ea-ada3-1285ba13d7a5&created=1789361503913&expire=1789363244478&c=0',
        }
        headers = {
            ...
            'pragma': 'no-cache',
            'priority': 'u=0, i',
            'referer': 'https://www.platinumrx.in/otc?label=Z',
            ...
            # 'cookie': 'platinumrx-device-id=c4f4d87f-86e8-4a88-87b4-e7a45cecb06c; platinumrx-session-id=03847e4a-2084-4150-87c8-025d9b71066a; _gcl_au=1.1.1386912106.1789029224; _ga=GA1.1.1020510031.1789029225; _ga_BHBBVCGVE0=GS2.1.s1789361506$o17$g1$t1789362342$j60$l0$h1716316719$da4THJHUtYw8-09PKcD8CrqgcohCDFd1nIw; _dd_s_v2=aid=47a24d2b-491d-423a-9130-315a19c20816&id=9080f951-f0ce-41ea-ada3-1285ba13d7a5&created=1789361503913&expire=1789363244478&c=0',
        }

        for id, url in urls:
            yield scrapy.Request(url=url,headers=headers,cookies=cookies,callback=self.parse,meta={"id": id})
    ...
    def parse(self, response):
        ...
        if not json_data:
            print("Product JSON-LD not found:", response.url)
            return
        try:
            data = json.loads(json_data)
        except json.JSONDecodeError:
            print("Invalid JSON-LD:", response.url)
            return
        ...
        cursor,conn=create_table()    
        cursor.execute("UPDATE platinumrx_health_products_url SET status = 1 WHERE id = %s",(ids,))
        conn.commit()
        cursor.close()
        conn.close()        
        yield item
